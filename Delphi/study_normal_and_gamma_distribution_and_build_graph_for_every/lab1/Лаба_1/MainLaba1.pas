...
uses
  ...
  TeeProcs, Chart, Menus;  { Math - это математическая библиотека}    {rang g -находится в библеотике math}
...
{это обработчик событий - вызов меню на экране}
end;
...
begin
 ...
 form1.PageControl1.ActivePageIndex:=0; {включаем page control на первую закладку}

 

 form1.Edit1.Text:='1000';           {вводим исходные значения}
 ...
 TitleText:='';      {загаловок графика}

 

 for i:=0 to 4 do form1.Chart1.Series[i].ShowInLegend:=false;    {обращение графикам возможно по двум системам имен:
 если графиков меньше 16 то к ним можно обращатья как к массиву}
...
{ Формирование нормально распределенной выборки случайных чисел }
 ...
 val(form1.Edit1.Text,j,k);     {типовой алгоритм для ввода числа с экрана}

 if k<>0     then j:=1000;    {если строка в число не преобразовывается то j=1000}

 if j<10     then j:=10;      {если j меньше 10, то минимальное значение j=10 j-минимальное число эксп}

 if j>100000 then j:=100000;     {если j больше 100000, то j=100000 - верхняя граница эксп}

 form1.Edit1.Text:=IntToStr(j);   {в edit1 -вводим исправленное значение j}
              {преобраз целое число в стр}   {j - число опытов}


 val(form1.Edit2.Text,s,k);      {s - ср кв отклонение}

 if k<>0    then s:=10;

 if s<0.001 then s:=0.001;

 STR(s:5:3,t); {преобразует число в строку, фомат числа 5 знаков, из них 3 после запятой}

 form1.Edit2.Text:=t;         {если для целых чисел знаков не хватит, то они будут добав автоматич, но три знака после запятой будеть всегда}
          {стр t выводим в edit2}


 val(form1.Edit3.Text,m,k);

 if k<>0 then m:=20; {m мат ожидание}
 ...
 form1.StringGrid1.Cells[0,0]:='№ п/п';   {stringGrid - текст таблица, Cells - ячейка}

 form1.StringGrid1.Cells[1,0]:='x непр. G';  { 1,0 - первый столбец нулевой строки}
 ...
 for i:=1 to j do
  ...
  begin
   ...
   x:=RandG(m,s);     {ген сл чисел, распред по норм зак, с мат ожид - m и ср кв откл s}

   if form1.StringGrid1.RowCount<i+1 then form1.StringGrid1.RowCount:=i+1;  {число строк в таблице stringgrid}
        {если число строк в таб < числа опытов, то мы прибавляем кней еще одну строку}      {нумерация строк всегда с нуля, потэтому на 1 больше, чем rowCount}
   form1.StringGrid1.Cells[0,i]:=IntToStr(i); {в нулевой столбец i-ая строка - выводим номер опыта}
...
{ Формирование нормально распределенной выборки
 ...
 for i:=1 to j do
  ...
  begin
   ...
   x:=Round(RandG(m,s)); {операция округления до ближайшего целого}
...
{ Построение гистограмм по выборкам нормально распределенных случайных чисел }
 ...
 val(form1.Edit1.Text,n,code);    {n - число опытов}
 if code<>0  then n:=1000;
 ...
 val(form1.Edit2.Text,s,code);    {s - ввод ср кв откл}
 ...
 val(form1.Edit3.Text,m,code);   { m- вводим мат ожидание }
 ...
 val(form1.Edit4.Text,k,code);  { к- число интервалов гистаграммы}
...
{ Очистка графиков, отключение легенд }
 ...
 for i:=1 to form1.StringGrid1.RowCount-1 do
  ...
  begin
   ...
   t:=form1.StringGrid1.Cells[0,i];{переписываем содерж ячейки}

   Val(t,x,k);

   if k<>0 then {k<>0 строку в число преобраз невозможно}

    begin

     StrPCopy(ADF,'В строке '+IntToStr(i)+' ошибка по х!!!'); {преобраз в нуль строку, правила языка СИ,
                                                               поэтому она не проверяет совместимость типов данных. Массив ADF исп как буфер}

     Application.MessageBox(ADF,'Ошибка чтения таблицы',0); {функция Win32 API кот выводит стандартное окно сообщ. Ошибка чтения таблицы - заголово к формы.
                                                               ADF содержимое формы. 0 на конце - число кнопок в табл, задается в двоичном виде.}
