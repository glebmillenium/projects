...
public class Daemon{
     ...
     * @throws IOException 
     */
    public static int main(int count, String[] Data_daemon, int max_count) throws FileNotFoundException, IOException {
        ...
        doProcessing(count,Data_daemon,max_count);
     ...
     * @throws IOException 
     */
    public static void doProcessing(int count, String[] Data_daemon,int max_count) throws FileNotFoundException, IOException
    {
        String way_in, way_from, rec_file, storage;
        way_in=Data_daemon[0];//Путь в файл в который записываем
        way_from=Data_daemon[1];//Откуда записываем
        rec_file=Data_daemon[2];//имя файла для записи метаданных
        storage=Data_daemon[3];
